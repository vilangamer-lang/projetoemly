...
function isLegacyJwtKey(value) {
  return /^eyJ[\w.-]+/.test(String(value || ""));
}
...
function formatMissingConfig(missing) {
  ...
  if (missing.length === 2) {
    return `${missing[0]} and ${missing[1]} are not configured`;
  }

  return `${missing.slice(0, -1).join(", ")}, and ${missing[missing.length - 1]} are not configured`;
}
...
async function supabaseFetch(path, { method = "GET", query, body } = {}) {
  ...
  if (missing.length) {
    const error = new Error(formatMissingConfig(missing));
...
function preparePatientPayload(input, { isCreate } = {}) {
  ...
  return {
    ...
    profile: {
      ...
      status: normalizeStatus(source.profile?.status || profile.status || "Online"),
      ...
      focus: source.profile?.focus || profile.focus || "Agenda, revisões e orientações da paciente.",
      ...
      careJourney: ensureArray(source.profile?.careJourney),
